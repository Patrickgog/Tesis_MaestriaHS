...
def load_pump_database(marca: str) -> Dict:
    ...
    """
    marcas_validas = ["Grundfos", "Ebara", "KSB", "Pedrollo"]
...
def filter_pumps_by_requirements(
    ...
    # Intento 1: Margen solicitado por el usuario
    bombas_compatibles = _search_in_db(database, caudal_lps, altura_m, margen_porcentaje)
    
    # Intento 2: Si no hay resultados, intentar con margen ampliado (auto-flexibilidad)
    if not bombas_compatibles:
        bombas_compatibles = _search_in_db(database, caudal_lps, altura_m, margen_porcentaje * 2)
        if bombas_compatibles:
            # Marcar que fue una búsqueda flexible
            for b in bombas_compatibles:
                b['_busqueda_flexible'] = True
...
def _search_in_db(database: Dict, caudal_lps: float, altura_m: float, margen: float) -> List[Dict]:
    """Función interna de búsqueda con margen específico"""
    resultados = []
    margen_factor = 1 + (margen / 100)
    q_min_req, q_max_req = caudal_lps / margen_factor, caudal_lps * margen_factor
    h_min_req, h_max_req = altura_m / margen_factor, altura_m * margen_factor
    ...
    for categoria in database.get('categorias', []):
        for bomba in categoria.get('bombas', []):
            ...
            if isinstance(bomba['rango_altura_optima_m'], list):
                ...
                h_opt_min, h_opt_max = h_opt * 0.8, h_opt * 1.25
            ...
            if (q_min_req <= q_cat_max and q_max_req >= q_cat_min) and \
               (h_min_req <= h_cat_max and h_max_req >= h_cat_min):
                ...
                b_info = bomba.copy()
                b_info['_score'] = score
                b_info['_categoria'] = categoria['categoria']
                b_info['_tipo'] = categoria.get('tipo', 'N/A')
                resultados.append(b_info)
    return resultados
