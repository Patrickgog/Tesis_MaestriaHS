        ...
        if mat_succion in ["PEAD", "HDPE (Polietileno)"]:
            ...
            if pead_data_succion:
                ...
                if series_disponibles:
                    serie_succion = st.selectbox(
                        ...
                        index=st.session_state.get('serie_succion_index', 0),
                        key="serie_succion",
            ...
            if diametros_hierro:
                dn_succion = st.selectbox(
                    ...
                    index=st.session_state.get('dn_succion_index', 0),
                    key="dn_succion",
            ...
            if diametros_hierro_fundido:
                dn_hierro_fundido_succion = st.selectbox(
                    ...
                    index=st.session_state.get('dn_hierro_fundido_succion_index', 0),
            ...
            if series_pvc:
                ...
                serie_pvc_succion_nombre = st.selectbox(
                    ...
                    index=st.session_state.get('serie_pvc_succion_nombre_index', 0),
                ...
                if diametros_pvc:
                    dn_pvc_succion = st.selectbox(
                        ...
                        index=st.session_state.get('dn_pvc_succion_index', 0),
                        key="dn_pvc_succion",
        ...
        if mat_impulsion in ["PEAD", "HDPE (Polietileno)"]:
            ...
            if pead_data_impulsion:
                ...
                if series_disponibles:
                    serie_impulsion = st.selectbox(
                        "Serie del Tubo",
                        options=series_disponibles,
                        index=st.session_state.get('serie_impulsion_index', 0),
                        key="serie_impulsion",
                        help="Selecciona la serie del tubo PEAD."
                    )
                    # Guardar el índice actual
            ...
            if diametros_hierro:
                dn_impulsion = st.selectbox(
                    "Diámetro Nominal (DN mm)",
                    options=diametros_hierro,
                    index=st.session_state.get('dn_impulsion_index', 0),
                    key="dn_impulsion",
                    help="Selecciona el diámetro nominal de la tubería de hierro dúctil."
                )
                # Guardar el índice actual
            ...
            if diametros_hierro_fundido:
                dn_hierro_fundido_impulsion = st.selectbox(
                    "Diámetro Nominal (DN mm)",
                    options=diametros_hierro_fundido,
                    index=st.session_state.get('dn_hierro_fundido_impulsion_index', 0),
            ...
            if series_pvc:
                ...
                serie_pvc_impulsion_nombre = st.selectbox(
                    "Serie del Tubo",
                    options=series_nombres,
                    index=st.session_state.get('serie_pvc_impulsion_nombre_index', 0),
                ...
                if diametros_pvc:
                    dn_pvc_impulsion = st.selectbox(
                        "Diámetro Nominal (DN mm)",
                        options=diametros_pvc,
                        index=st.session_state.get('dn_pvc_impulsion_index', 0),
    ...
    with st.expander("🏭 Catálogo de Bombas Comerciales (Prediseño)", expanded=expander_bomba_abierto):
        ...
        **Herramienta de Prediseño**: Selecciona bombas comerciales (Grundfos, Ebara) basadas en tus 
        ...
        with col_criterios:
            ...
            with col_q:
                # Usar caudal de diseño POR BOMBA del proyecto
                n_bombas = st.session_state.get('num_bombas', 1)
                q_total = st.session_state.get('caudal_lps', 51.0)
                caudal_default = q_total / n_bombas if n_bombas > 0 else q_total
                
                caudal_diseno = st.number_input(
                    "Caudal de Diseño POR BOMBA (L/s)",
                    value=caudal_default,
                    min_value=0.1,
                    step=0.1,
                ...
                if n_bombas > 1:
                    st.caption(f"💡 Info: {q_total:.2f} L/s total / {n_bombas} bombas")
            
            with col_h:
                # Usar ADT total si está disponible
                altura_default = st.session_state.get('adt_total', 100.0)
