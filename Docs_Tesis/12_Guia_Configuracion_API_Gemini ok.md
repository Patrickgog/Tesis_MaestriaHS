...
### 📌 Método Alternativo (Solo para Desarrolladores)

**Si está modificando el código fuente** de la aplicación y quiere configurar una API key permanente:

<details>
<summary>Haga clic para ver las instrucciones avanzadas</summary>
...
2. **Agregue su API key**:
```toml
GEMINI_API_KEY = "AIzaSy_TU_API_KEY"
```

3. **Guarde y reinicie** la aplicación

#### Para Streamlit Cloud (si despliega su propia versión)

1. En Streamlit Cloud, vaya a **Settings → Secrets**
2. Agregue:
```toml
GEMINI_API_KEY = "AIzaSy_TU_API_KEY"
```
3. Guarde y la app se reiniciará

**⚠️ CRÍTICO**: Si usa este método, asegúrese de agregar `.streamlit/secrets.toml` a su `.gitignore`:
```
# NO SUBIR secrets
...
## SECCIÓN 6: MEJORES PRÁCTICAS Y CONSEJOS

### Seguridad de la API Key

✅ **HACER**:
- Usar `secrets.toml` para almacenamiento local
- Usar Streamlit Secrets para producción
- Agregar `.streamlit/secrets.toml` a `.gitignore`
- Rotar la API key periódicamente (cada 3-6 meses)

❌ **NO HACER**:
- Nunca hardcodear la API key en el código fuente
- Nunca subir la API key a GitHub
- Nunca compartir la API key públicamente
- No usar la misma API key en múltiples proyectos

### Optimizar Uso de Cuota

**Consejos para no agotar la cuota gratuita**:

1. **No analice cada cambio menor**
   - Use el análisis IA solo cuando el diseño esté mayormente completo
   - Un análisis por diseño finalizado es suficiente

2. **Cache local** (implementado en la app):
   - La aplicación cachea resultados de Gemini
   - Si analiza el mismo diseño dos veces, usa el cache

3. **Monitoreo de uso**:
   - Visite https://aistudio.google.com/app/apikey
   - Vea su consumo actual
   - Ajuste uso si se acerca a límites

### Mejorar Calidad de Respuestas

El **prompt** enviado a Gemini ya está optimizado en la aplicación, pero puede personalizar:

**Archivo**: `ui/ai_module.py`  
**Función**: `generar_analisis_ia_gemini()`

**Modificación ejemplo** (para respuestas más técnicas):
```python
prompt = f"""
Actúa como ingeniero hidráulico senior con PhD y 25 años de experiencia.
Analiza este diseño usando terminología técnica avanzada.
Cita normas específicas (ASME B73.1, HI 9.6.1, ISO 5199).
...
"""
```
...
**Opción 1**: Cada usuario usa su propia API key
- Cada persona obtiene su propia key gratuita
- Configura su propio `secrets.toml` local

**Opción 2**: Despliegue en Streamlit Cloud
- Usted despliega la app en https://streamlit.io/cloud
- Configura secrets en Streamlit (solo usted los ve)
- Otros usan la app sin necesitar API key
...
**Soporte**:
- Foro de Gemini: https://discuss.ai.google.dev/
